...
int main(int argc, char** argv)
{
	...
	ctrl_points.push_back(point(5.f, 0.f, 0.f));
	ctrl_points.push_back(point(0.f, 5.f, 0.f));
	ctrl_points.push_back(point(0.f, -5.f, 0.f));

	int num_eval = 128;
	std::vector<point> curve = EvaluateBezierCurve(ctrl_points, num_eval);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 projection = glm::perspective(glm::radians(45.f), (float)WIDTH / (float)HEIGHT, .01f, 500.f);
